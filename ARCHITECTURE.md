...
The `Container` class centralizes dependency wiring. It takes a shared `DataSource` (public schema) and a `TenantDataSourceManager` (per-tenant schemas). Global services (auth, tenant) use the shared DataSource. Tenant-scoped services (chatbot, knowledge, etc.) use a `DataSource` resolved by `tenantSlug`:
...
Flow: `routes/api.ts` → `routes/api/widget-settings.ts` → `createChatbotModule(container)` → `ChatbotController`
...
// routes/api/widget-settings.ts — authMiddleware + requireTenant
...
- **Tenant-scoped data** (chatbot, knowledge): Per-tenant schema (`tenant_<slug>`), resolved via `TenantDataSourceManager`
...
|------------|---------|--------|
| `authMiddleware` | Verify JWT, set `user` in context | `/tenants`, all chatbot admin routes |
| `requireTenant` | Require `tenantId` in JWT (non-null) | All chatbot admin routes |
...
```
modules/chatbot/                    # Tenant-scoped module
├── entities/
│   └── widget-settings.entity.ts   # Extends TenantAwareEntity
│   └── chatbot-settings.entity.ts
│   └── chatbot-form-field.entity.ts
│   └── chatbot-session.entity.ts
│   └── chatbot-session-value.entity.ts
├── repositories/
│   └── widget-settings.repository.ts
├── serializers/
│   └── chatbot.serializer.ts       # DTO transformer (serialize + collection)
├── validators/
│   └── chatbot.validator.ts        # Zod schemas
├── interfaces/
│   └── chatbot.interface.ts        # Type contracts
├── enums/
│   └── chatbot.enum.ts             # Enumerations
├── chatbot.controller.ts           # Handler methods (uses serviceFactory)
├── chatbot.service.ts              # Business logic
└── chatbot.module.ts               # DI wiring (returns Controller)
